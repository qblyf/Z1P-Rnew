...
**SmartMatch.tsx 和 TableMatch.tsx**:
```typescript
// 修改前
...
```
=== 开始加载SPU和SKU规格数据 ===
已加载 10000 个SPU，过滤 23 个无品牌SPU，有效: 9977，总计: 9977
=== SPU数据加载完成 ===
总加载: 10000 个SPU
过滤无品牌: 23 个SPU
有效SPU: 9977 个
...
1. **匹配准确性**: 品牌是匹配的关键字段，无品牌SPU无法参与品牌匹配
2. **性能优化**: 减少无效数据，提高匹配速度
3. **数据质量**: 无品牌SPU通常是数据不完整的记录
...
- 修复方案1：使用正确的 `'p."created_at"'` key
- 修复方案2：过滤无品牌SPU，提高匹配质量
- 影响范围：智能匹配和表格匹配功能
- 测试状态：构建通过，需要浏览器功能测试
