...
import { editParamsDefinition } from '@zsqk/z1-sdk/es/z1p/params-definition';
 ...
 * @author Guo Yafang
 */
export function EditParam(props: {
  paramInfo: ParamsDefinition;
  onOk: (e: 'cancal' | 'ok') => void;
}): JSX.Element {
  const { token } = useTokenContext();
  const { paramInfo, onOk } = props;
  // 参数值列表
  ...
  >(
    paramInfo.options.map((option: string, index: number) => {
      return {
        key: index + 1,
        name: option,
      };
  ...
  const [count, setCount] = useState(paramInfo.options.length);
  // 参数组
  const [group, setGroup] = useState<number>(paramInfo.group);
  // 参数名
  const [name, setName] = useState<string>(paramInfo.name);
  // 权重值
  const [weight, setWeight] = useState<number | undefined>(paramInfo.weight);
  // 适用分类
  const [categoryIDs, setCategoryIDs] = useState<CateID[] | undefined>(
    paramInfo.categoryIDs
  );
